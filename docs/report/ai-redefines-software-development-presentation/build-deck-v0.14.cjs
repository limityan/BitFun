...
const slidesDir = path.join(outDir, "agents_are_the_new_Agile-slides-png-v0.14");
const pptxPath = path.join(outDir, process.env.PPTX_FILE || "agents_are_the_new_Agile-v0.14.pptx");
const notesPath = path.join(outDir, "agents_are_the_new_Agile-speaker-notes-v0.14.md");
const readmePath = path.join(outDir, "README.md");
const contactSheetPath = path.join(outDir, "agents_are_the_new_Agile-preview-contact-sheet-v0.14.png");
...
const finalContentV12 = [
  ...
  },
  {
    title: "目录",
    time: "约 0.7 分钟",
    focus: "用四个模块组织全篇：软件工程变革、速度的背后、工程质量与治理、开发者角色。",
  ...
  {
    title: "ADE 范围",
    subtitle: "以工程控制面贯通研发全生命周期。",
    time: "约 1.2 分钟",
    focus: "说明 BitFun/ADE 不只是更强 IDE，而是 Agentic SDLC 控制面：任务契约、隔离执行、验证矩阵、Artifact 包、反馈记忆。",
  ...
  {
    title: "需求到任务",
    subtitle: "把模糊需求转成任务契约：目标、非目标、验收标准和风险边界先被定义。",
    time: "约 1.3 分钟",
    focus: "用研发链路第一个节点讲清：AI 不是从“写代码”开始，而是从可执行任务契约开始。",
  ...
  {
    title: "评审到交付",
    subtitle: "当 AI 放大变更吞吐，评审对象要从最终 diff 扩展为可追责的 Artifact 包。",
    time: "约 1.4 分钟",
...
slides.forEach((slide, i) => Object.assign(slide, finalContentV12[i]));
...
function slideAgendaV10(slide, index) {
  const body = `
    ...
    ${agendaItemV10(210, 278, "01", "软件工程变革", "从 BitFun 高吞吐经验看生命周期交付件变化。", "03")}
    ${agendaItemV10(336, 428, "02", "速度的背后", "研究和竞品都指向系统化承接。", "04-05")}
    ${agendaItemV10(210, 578, "03", "工程质量与治理", "从产品范围到研发全过程节点。", "06-10")}
...
function slideThanksV10(slide, index) {
  const body = `
    ...
    ${thanksLandingV10(244, 704, "02", "信任如何定价？", "当产出接近无限，稀缺的是证据、责任与回滚", L.orange)}
...
function slideAdeLifecycleMapV12(slide, index) {
  const body = `
    ...
    ${lifecycleStageV12(468, 392, "02", "任务契约", "目标 / 非目标", "验收与风险边界", L.blue, "file")}
...
function slideRequirementContractV12(slide, index) {
  const body = `
    ...
    ${v02Takeaway("任务契约定义 Agent 执行的准入条件：先明确目标、边界、验收与风险。")}
  `;
  return svgBaseV5(slide, index, body);
}
...
function contractPanelV12(x, y) {
  return `
    ...
    <text x="${x + 134}" y="${y + 88}" class="label5" style="font-size:31px">任务契约</text>
...
function slideReviewDeliveryV12(slide, index) {
  const body = `
    ...
    <text x="960" y="522" class="num5" text-anchor="middle" style="font-size:40px;fill:${L.ink}">Artifact 包 = PR 判断上下文</text>
    ...
    ${v02Takeaway("Artifact 包应由平台自动生成、绑定 PR/发布记录，并提供人类可读摘要。")}
  `;
  return svgBaseV5(slide, index, body);
}
...
本目录保留原版演讲材料、V0.2、V0.3、V0.4、V0.5、V0.6、V0.8、V0.9、V0.10、V0.11、V0.12、V0.13 版本，并新增 V0.14 版本。V0.14 继续使用整页图片式生成链路，不做元素化编辑版改造；本版重点强化“可治理 Agentic SDLC”主线，明确任务契约、隔离执行、验证矩阵、Artifact 包、反馈记忆等工程控制面关键构件，并补强 BitFun/ADE 的产品定位、净收益度量与 Artifact 管理机制。
