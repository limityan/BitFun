...
const slidesDir = path.join(outDir, "agents_are_the_new_Agile-slides-png-v0.7");
const pptxPath = path.join(outDir, process.env.PPTX_FILE || "agents_are_the_new_Agile-v0.7.pptx");
const notesPath = path.join(outDir, "agents_are_the_new_Agile-speaker-notes-v0.7.md");
const readmePath = path.join(outDir, "README.md");
const contactSheetPath = path.join(outDir, "agents_are_the_new_Agile-preview-contact-sheet-v0.7.png");
...
const slides = [
  ...
  {
    section: "03 / ENGINEERING BASE",
    title: "BitFun：把 Agent 能力放进工程控制面",
    subtitle: "AGENTS.md、工具准入、权限隔离和验证矩阵，共同保护 AI 交付能力。",
    time: "约 1.3 分钟",
    focus: "用 BitFun 本地工程机制说明运行基座不是概念，而是规则、工具、权限和验证的组合。",
    question: "同样的模型，为什么放进不同工程系统里，可靠性会差很多？",
    script:
      "把这个观点落回 BitFun，会看到它不是一句抽象的“用好上下文”。第一层是规则上下文，仓库级和模块级 AGENTS.md、架构文档、日志规范、remote compatibility 规则，会先告诉 Agent 什么能改、什么边界不能碰。第二层是工具准入，工具执行链里有 allowed_tools、runtime tool restrictions，折叠工具还要求先读取 GetToolSpec，避免模型在不知道完整约束时直接调用。第三层是路径和权限隔离，runtime artifact 不能逃逸 root，MiniApp 的 fs、shell、net 走 allowlist，权限变更还会标出高风险 diff。第四层是验证矩阵，前端、Deep Review、共享 Rust、桌面集成分别有最小验证路径。这样看，BitFun 的工程控制面不是多写几条提示词，而是把规则、工具、权限和验证做成 Agent 可以读取、系统可以拦截、人可以复核的结构。",
  ...
  },
  {
    ...
    subtitle: "允许 Agent 多路径探索，但把放行收敛到可复现证据和阶段门禁。",
    time: "约 1.3 分钟",
    focus: "解释可控系统如何收敛不可控过程：沙箱、权限、证据包和阶段门禁各自承担责任。",
    question: "如果每一步都有 99% 正确率，十步之后系统还可信吗？",
    script:
  ...
  },
  {
    ...
    subtitle: "不是聊天产物堆叠，而是任务、环境、证据、评审和门禁的连续交付对象。",
    time: "约 1.3 分钟",
    focus: "给出一条可执行工作流：任务入口、隔离执行、证据包、独立评审、阶段放行。",
    question: "如果多个 Agent 同时参与，团队靠什么判断一个变更可以继续前进？",
    script:
    ...
    transition: "这个流程里最关键的交付对象，就是证据包。",
    render: slideBitfunV02,
  },
  {
    section: "04 / OPERATING MODEL",
    title: "证据包：让团队审查对象变小",
    subtitle: "人不需要重放每一步，但必须能审查差异、测试、风险和责任。",
    time: "约 1.2 分钟",
    focus: "把协作对象从代码扩展到任务、Agent、工具执行、证据包和责任链。",
    question: "一个 AI 生成的 PR，至少应该带上哪些证据才值得评审？",
    script:
    ...
    transition: "证据包解决可审查性，下一步还要解决优先级：哪些变更该快，哪些必须慢。",
    render: slideCoverV02,
  },
  {
    section: "04 / OPERATING MODEL",
...
const positiveToneOverrides = [
  ...
  {
    index: 2,
    title: "单月 10W+ 行代码之后，收益如何被接住？",
    subtitle: "高速产出是真实增益，也是一轮工程系统升级机会。",
    focus: "用 BitFun 统计把问题落到真实场景：AI 已经带来产能跃迁，工程系统要把产能转成可持续交付。",
    question: "如果一个项目单月写出 10W+ 行代码，团队怎样把这份产能变成长期收益？",
    script:
    ...
    transition: "单个项目的现象还不够，需要看看外部研究是否也指向“收益来自系统化承接”。",
  },
  {
    index: 3,
    script:
  ...
  {
    index: 8,
    title: "速度收益：把返工变成可管理队列",
    subtitle: "处理办法不是给 AI 降速，而是量化净收益、分流风险、闭环修复。",
    focus: "给出三个落地动作：量化净时间、按风险分流、让失败回到 Agent 与 CI 闭环，从而放大 AI 正收益。",
    script:
    ...
    transition: "速度收益被稳定承接之后，质量治理就能从负担变成放大器。",
  },
  {
    index: 9,
    section: "05 / 工程质量",
    title: "工程质量：责任链、证据链、Artifact 链",
    subtitle: "开源和大厂不是两套逻辑，都是把质量要求变成可复核工件。",
    focus: "区分开源与大厂质量要求，但落到共同检查面：责任链、证据链、Artifact 链，并连接 BitFun 的工程保护机制。",
    ...
    transition: "这些链路再往前走，就会变成 DFX、TDD 和 Code Review 的可执行工程协议。",
...
function sectionWatermark(index) {
  ...
  if (!item) return "";
  const color = item.n === "03" || item.n === "04" || item.n === "05" ? L.orange : L.blue;
  return `
...
function sectionWatermarkInfo(index) {
  const page = index + 1;
  if (page >= 3 && page <= 4) return { n: "01", title: "问题与证据" };
  if (page >= 5 && page <= 6) return { n: "02", title: "工程基座" };
  if (page >= 7 && page <= 9) return { n: "03", title: "落地框架" };
  if (page >= 10 && page <= 11) return { n: "04", title: "质量治理" };
  if (page >= 12 && page <= 12) return { n: "05", title: "角色与行动" };
  return null;
}
...
function v02Takeaway(textValue, width = 1320, y = 954) {
  ...
  return `
    ...
    <text x="${x + width / 2 + 26}" y="${y + 32}" class="body5" text-anchor="middle" dominant-baseline="central" alignment-baseline="middle" style="font-size:27px">${esc(textValue)}</text>
  `;
}
...
function slideTitleV02(slide, index) {
  const body = `
    ...
    ${velocityQualityWatermarkV07(1110, 324, { opacity: 0.34, scale: 1.05 })}
...
function slideAgendaV02(slide, index) {
  const body = `
    ${v5Header("目录", slide.title, "", 72)}
    ${agendaItem5(150, 370, "01", "现实问题", "代码产能放大后，验证与维护成为真瓶颈。", "03", L.blue)}
    ${agendaItem5(1010, 448, "02", "外部证据", "企业研究、顶会论文、行业判断共同指向系统收益。", "04", L.blue)}
    ${agendaItem5(150, 606, "03", "治理框架", "运行基座、证据放行、Issue 到 PR 工作流。", "05-09", L.orange)}
    ${agendaItem5(1010, 684, "04", "职责变化", "团队建系统，个人守判断。", "10-13", L.orange)}
    <text x="960" y="914" class="muted5" text-anchor="middle" style="font-size:25px">现实问题 → 外部证据 → 治理框架 → 职责变化</text>
  `;
  return svgBaseV5(slide, index, body);
}
...
function slideShockV02(slide, index) {
  const body = `
    ${v5Header("01 / 现实问题", slide.title, slide.subtitle, 60)}
    <rect x="116" y="398" width="522" height="360" rx="20" fill="${L.paper}" stroke="${L.line2}" stroke-width="2" filter="url(#paperShadow)"/>
    <text x="174" y="446" class="small5" style="font-size:22px;fill:${L.blue};font-weight:900">BitFun 开发经验</text>
    <text x="174" y="514" class="num5" style="font-size:52px;fill:${L.blue}">单月 10W+ 行代码</text>
    <path d="M174 592 H586" stroke="${L.line2}" stroke-width="3"/>
    <text x="174" y="636" class="body5" style="font-size:27px">不是炫耀数字，而是压力测试</text>
    <text x="174" y="684" class="small5" style="font-size:21px">验证、评审、协作、维护是否同步扩容？</text>
    <text x="174" y="730" class="small5" style="font-size:19px;fill:${L.muted};font-weight:720">统计口径：作者提交历史，非代码质量结论。</text>
    <path d="M666 560 H742" stroke="${L.blue}" stroke-width="5" fill="none" marker-end="url(#arrowBlue)"/>
    ${repoStatsCaptureV05(784, 348)}
    <rect x="720" y="648" width="950" height="188" rx="18" fill="${L.paper}" stroke="${L.line2}" stroke-width="2" filter="url(#paperShadow)"/>
    <text x="780" y="690" class="small5" style="font-size:23px;fill:${L.orange};font-weight:900">快在哪里：不是“写得多”，而是反馈路径变短</text>
    ${miniFlowV05(800, 748, "想法 / Issue", "隔离执行", "原型 / PR", "验证与评审")}
    ${v02Takeaway("BitFun 的价值不在代码量本身，而在暴露了 AI 开发需要新的治理口径。", 1420)}
  `;
  return svgBaseV5(slide, index, body);
}
...
function slideCoverV02(slide, index) {
  const body = `
    ${v5Header("04 / 证据对象", slide.title, slide.subtitle, 64)}
    <text x="132" y="396" class="small5" style="font-size:23px;fill:${L.blue};font-weight:900">代码生产放大以后，团队要审查的是可压缩、可复核的证据对象</text>
    ${collabRowV05(132, 486, "01", "任务规格", "目标、非目标、影响范围和验收标准必须先写清楚。", L.blue, "terminal")}
    ${collabRowV05(132, 616, "02", "执行轨迹", "记录关键工具调用、失败尝试、上下文差异和权限边界。", L.orange, "team")}
    ${collabRowV05(132, 746, "03", "放行证据", "人不重放每步操作，而是审 diff、测试、风险、owner。", L.blue, "check")}
    <path d="M806 612 C850 586 900 586 944 612" stroke="${L.blue}" stroke-width="5" fill="none" marker-end="url(#arrowBlue)"/>
    ${evidenceCaptureV05(986, 478)}
    ${v02Takeaway("证据包把 AI 的执行过程，压缩成团队可以评审、追责和复盘的对象。", 1420)}
  `;
  return svgBaseV5(slide, index, body);
}
...
function evidenceCaptureV05(x, y) {
  return `
    <rect x="${x}" y="${y}" width="680" height="254" rx="18" fill="#111827" stroke="${L.line2}" stroke-width="2" filter="url(#paperShadow)"/>
    <rect x="${x}" y="${y}" width="680" height="40" rx="18" fill="#1F2937"/>
    <circle cx="${x + 30}" cy="${y + 21}" r="6" fill="#EF4444"/>
    <circle cx="${x + 52}" cy="${y + 21}" r="6" fill="#F59E0B"/>
    <circle cx="${x + 74}" cy="${y + 21}" r="6" fill="#22C55E"/>
    <text x="${x + 104}" y="${y + 11}" class="small5" style="font-size:17px;fill:#D1D5DB;font-weight:900">Evidence Packet · docs / checks excerpt</text>
    <text x="${x + 34}" y="${y + 66}" class="micro5" style="font-size:18px;fill:#93C5FD">docs/architecture/deep-review.md</text>
    <text x="${x + 34}" y="${y + 100}" class="small5" style="font-size:18px;fill:#E5E7EB;font-weight:820">changed paths · diff stats · risk tags · packet ids</text>
    <text x="${x + 34}" y="${y + 136}" class="micro5" style="font-size:18px;fill:#93C5FD">AGENTS.md / Verification</text>
    <text x="${x + 34}" y="${y + 170}" class="small5" style="font-size:18px;fill:#E5E7EB;font-weight:820">pnpm lint + type-check + test:run</text>
    <text x="${x + 34}" y="${y + 202}" class="small5" style="font-size:18px;fill:#E5E7EB;font-weight:820">cargo test -p bitfun-core deep_review</text>
    <rect x="${x + 34}" y="${y + 222}" width="316" height="26" rx="13" fill="#064E3B"/>
    <text x="${x + 54}" y="${y + 228}" class="micro5" style="font-size:16px;fill:#BBF7D0;font-weight:900">证据不是口头说明，而是可复核产物</text>
  `;
}
...
function slideExplorationV02(slide, index) {
  const body = `
    ${v5Header("03 / 工程基座", slide.title, slide.subtitle, 64)}
    ...
    <rect x="120" y="394" width="1680" height="382" rx="20" fill="${L.paper}" stroke="${L.line2}" stroke-width="2" filter="url(#paperShadow)"/>
    <path d="M540 430 V742 M960 430 V742 M1380 430 V742" stroke="${L.line2}" stroke-width="2"/>
    ${controlLaneV07(160, 444, "01", "规则上下文", "让 Agent 先知道边界", ["仓库 / 模块 AGENTS.md", "架构、日志、remote 规则", "先有事实，再有生成"], L.blue, "file")}
    ${controlLaneV07(580, 444, "02", "工具准入", "工具不是无限开放", ["allowed_tools 白名单", "runtime restrictions", "GetToolSpec 后再调用"], L.orange, "grid")}
    ${controlLaneV07(1000, 444, "03", "权限隔离", "影响面必须可控", ["artifact 不能逃逸 root", "fs / shell / net allowlist", "高风险权限 diff"], L.blue, "shield")}
    ${controlLaneV07(1420, 444, "04", "验证矩阵", "按风险选择验证", ["Web lint / type / test", "Deep Review 专项测试", "Rust / Desktop 分层检查"], L.orange, "check")}
    <path d="M210 842 H1710" stroke="${L.line2}" stroke-width="4" stroke-linecap="round"/>
    <path d="M210 842 H1710" stroke="${L.blue}" stroke-width="4" stroke-linecap="round" opacity="0.50"/>
    <circle cx="210" cy="842" r="12" fill="${L.blue}"/>
    <circle cx="710" cy="842" r="12" fill="${L.blue}"/>
    <circle cx="1210" cy="842" r="12" fill="${L.orange}"/>
    <circle cx="1710" cy="842" r="12" fill="${L.orange}"/>
    <text x="210" y="884" class="small5" text-anchor="middle" style="font-size:21px;fill:${L.muted};font-weight:850">任务进入</text>
    <text x="710" y="884" class="small5" text-anchor="middle" style="font-size:21px;fill:${L.muted};font-weight:850">受控执行</text>
    <text x="1210" y="884" class="small5" text-anchor="middle" style="font-size:21px;fill:${L.muted};font-weight:850">证据产出</text>
    <text x="1710" y="884" class="small5" text-anchor="middle" style="font-size:21px;fill:${L.muted};font-weight:850">人工复核</text>
    ${v02Takeaway("BitFun 的落地点：不是把模型接进仓库就结束，而是把模型放进一套工程控制面。", 1340)}
  `;
  return svgBaseV5(slide, index, body);
}
...
function slideReliabilityV02(slide, index) {
  const body = `
    ${v5Header("03 / 关键机制", slide.title, slide.subtitle, 66)}
...
function slideReliabilityBridgeV08(slide, index) {
  const body = `
    ${v5Header("03 / 关键机制", slide.title, slide.subtitle, 58)}
    <rect x="1280" y="336" width="500" height="64" rx="32" fill="#FFF6EE" stroke="${L.orange}" stroke-width="2"/>
    <text x="1530" y="358" class="small5" text-anchor="middle" style="font-size:24px;fill:${L.orange};font-weight:900">单步可信 ≠ 长链路可信</text>
    <text x="1530" y="386" class="micro5" text-anchor="middle" style="font-size:16px;fill:${L.muted};font-weight:820">0.99^10 ≈ 0.90，需要工程收敛</text>

    <rect x="110" y="440" width="450" height="330" rx="20" fill="${L.paper}" stroke="${L.line2}" stroke-width="2" filter="url(#paperShadow)"/>
    <circle cx="186" cy="520" r="46" fill="${L.blue}" opacity="0.10"/>
    ${v5Icon("terminal", 158, 492, 56, L.blue)}
    <text x="258" y="486" class="num5" style="font-size:36px;fill:${L.blue}">01</text>
    <text x="326" y="491" class="small5" style="font-size:24px;fill:${L.blue};font-weight:900">概率探索层</text>
    <text x="162" y="600" class="num5" style="font-size:34px;fill:${L.ink}">让 Agent 放开探索</text>
    <path d="M162 652 H508" stroke="${L.blue}" stroke-width="4" stroke-linecap="round" opacity="0.72"/>
    ${miniChipV08(166, 686, "沙箱", L.blue)}
    ${miniChipV08(284, 686, "工具", L.blue)}
    ${miniChipV08(402, 686, "重试", L.blue)}
    ${miniChipV08(166, 730, "权限", L.blue)}
    ${miniChipV08(284, 730, "trace", L.blue)}
    ${miniChipV08(402, 730, "回注", L.blue)}

    <path d="M590 604 H682" stroke="${L.blue}" stroke-width="5" fill="none" marker-end="url(#arrowBlue)" opacity="0.88"/>

    <rect x="710" y="410" width="500" height="390" rx="24" fill="${L.paper}" stroke="${L.line2}" stroke-width="2.5" filter="url(#paperShadow)"/>
    <text x="760" y="474" class="small5" style="font-size:24px;fill:${L.orange};font-weight:900">02 / 确定性中间件</text>
    <text x="760" y="538" class="num5" style="font-size:38px;fill:${L.ink}">把过程压缩成证据</text>
    ${middleStepV08(770, 610, "记录", "trace / tool calls", L.blue)}
    ${middleStepV08(770, 672, "约束", "权限 / 路径 / 环境", L.orange)}
    ${middleStepV08(770, 734, "验证", "测试 / 风险 / 回滚", L.blue)}

    <path d="M1240 604 H1332" stroke="${L.orange}" stroke-width="5" fill="none" marker-end="url(#arrowGray)" opacity="0.88"/>

    <rect x="1360" y="440" width="450" height="330" rx="20" fill="${L.paper}" stroke="${L.line2}" stroke-width="2" filter="url(#paperShadow)"/>
    <circle cx="1436" cy="520" r="46" fill="${L.orange}" opacity="0.12"/>
    ${v5Icon("shield", 1408, 492, 56, L.orange)}
    <text x="1508" y="486" class="num5" style="font-size:36px;fill:${L.orange}">03</text>
    <text x="1576" y="491" class="small5" style="font-size:24px;fill:${L.orange};font-weight:900">放行门禁</text>
    <text x="1412" y="600" class="num5" style="font-size:34px;fill:${L.ink}">只在转换点判断</text>
    <path d="M1412 652 H1758" stroke="${L.orange}" stroke-width="4" stroke-linecap="round" opacity="0.72"/>
    ${gateStepV08(1418, 690, "计划", "实现", L.orange)}
    ${gateStepV08(1604, 690, "实现", "评审", L.orange)}
    ${gateStepV08(1418, 742, "评审", "合并", L.orange)}
    ${gateStepV08(1604, 742, "发布", "复盘", L.orange)}
...
function slideExternalSignalsV02(slide, index) {
  const body = `
    ${v5Header("02 / 外部证据", slide.title, slide.subtitle, 66)}
    ...
    <rect x="236" y="864" width="1448" height="62" rx="14" fill="${L.bg}" stroke="${L.line2}" stroke-width="2"/>
    <text x="960" y="895" class="small5" text-anchor="middle" dominant-baseline="central" alignment-baseline="middle" style="font-size:25px;fill:${L.ink};font-weight:900">核心判断：AI 研发收益 = 架构适配 + 生产可控 + 平台治理 + 质量约束</text>
    ${v02Takeaway("AI 提升的是局部产能；能不能变成净交付，取决于工程系统。", 1420, 956)}
  `;
  return svgBaseV5(slide, index, body);
}
...
function slideLifecycleV02(slide, index) {
  const body = `
    ${v5Header("04 / 风险分流", slide.title, slide.subtitle, 58)}
    ...
    <text x="960" y="841" class="body5" text-anchor="middle" dominant-baseline="central" alignment-baseline="middle" style="font-size:27px"><tspan style="fill:${L.blue};font-weight:900">可执行方向：</tspan><tspan>PR 风险评分 + 证据包 + CI 失败回注 + Trace 归因</tspan></text>
...
function slideQualityV02(slide, index) {
  const body = `
    ${v5Header("05 / BitFun 演进", slide.title, slide.subtitle, 60)}
...
function slideQualityChainsV08(slide, index) {
  const body = `
    ${v5Header("05 / 工程质量", slide.title, slide.subtitle, 58)}
    ${qualityPanelV08(110, 410, "开源高质量要求", "提升对提交质量的信任", [
      "贡献者对代码负责",
      "维护者看方向、可读性、测试",
      "小批量变更 + 可复核证据",
    ], L.blue, "team")}

    <rect x="710" y="390" width="500" height="350" rx="26" fill="${L.paper}" stroke="${L.line2}" stroke-width="2.4" filter="url(#paperShadow)"/>
    <text x="960" y="438" class="small5" text-anchor="middle" style="font-size:24px;fill:${L.muted};font-weight:900">共同检查面</text>
    ${chainPillV08(802, 494, "责任链", "谁决策 / 谁审核 / 谁承担", L.blue)}
    ${chainPillV08(802, 582, "证据链", "测试 / 日志 / trace / 回滚", L.ink)}
    ${chainPillV08(802, 670, "Artifact 链", "代码 / 配置 / 依赖 / 发布物", L.orange)}

    ${qualityPanelV08(1290, 410, "大厂复杂交付", "稳定工件贯穿系统", [
      "需求 / 设计 / API / SLO",
      "证据贯穿计划、发布与复盘",
      "owner、回滚、合规可追踪",
    ], L.orange, "grid")}

    <rect x="190" y="790" width="1540" height="96" rx="18" fill="${L.bg}" stroke="${L.line2}" stroke-width="2"/>
    <text x="236" y="822" class="small5" style="font-size:22px;fill:${L.blue};font-weight:900">落回 BitFun</text>
    <text x="236" y="860" class="body5" style="font-size:26px;fill:${L.ink};font-weight:900">AGENTS.md 定责任边界，Evidence Packet 接证据链，artifact root / contract tests / boundary check 保护工件链。</text>

    ${v02Takeaway("质量治理不是多一道审批，而是把责任、证据和工件变成可复核工作。", 1320)}
  `;
  return svgBaseV5(slide, index, body);
}
...
function slideFutureQualityV02(slide, index) {
  const body = `
    ${v5Header("05 / 工程协议", slide.title, slide.subtitle, 58)}
    ${v02Panel(100, 390, 560, 250, "shield", "DFX：约束前移", ["AI 生成越快，架构债越快；", "性能、安全、可观测要进检查面。"], L.blue)}
    ${v02Panel(680, 390, 560, 250, "check", "TDD：证据先行", ["先定义失败证据和验收边界，", "再让 Agent 在边界内生成和修复。"], L.orange)}
    ${v02Panel(1260, 390, 560, 250, "file", "Review：人做仲裁", ["AI 可做第一轮检查，", "人判断风险、owner、回滚和证据。"], L.blue)}
    <rect x="402" y="770" width="1116" height="76" rx="16" fill="${L.bg}" stroke="${L.line2}" stroke-width="2"/>
    <text x="960" y="792" class="small5" text-anchor="middle" style="font-size:26px;font-weight:900">AI 不是取消 DFX、TDD、Review，而是把它们推向前置、显性和可执行。</text>
...
function slideBitfunV02(slide, index) {
  const body = `
    ${v5Header("04 / 工作流落地", slide.title, slide.subtitle, 58)}
    ...
    ${v02Takeaway("落地重点不是多一个聊天入口，而是形成任务、环境、证据、评审和门禁的连续交付流。", 1540)}
  `;
  return svgBaseV5(slide, index, body);
}
...
function slideRoleV02(slide, index) {
  const body = `
    ${v5Header("06 / 角色定位", slide.title, slide.subtitle, 64)}
...
function slideResponsibilityV06(slide, index) {
  const body = `
    ${v5Header("06 / 职责变化", slide.title, slide.subtitle, 58)}
    ...
    <text x="166" y="430" class="small5" style="font-size:24px;fill:${L.blue};font-weight:900">TEAM RESPONSIBILITY</text>
    ...
    ${responsibilityRowV06(168, 636, "02", "证据交付", "把测试、CI、trace、review 变成交付的一部分", L.blue)}
    ...
    <text x="1066" y="430" class="small5" style="font-size:24px;fill:${L.orange};font-weight:900">PERSONAL RESPONSIBILITY</text>
...
function slideThanksV02(slide, index) {
  const body = `
    ...
    ${v02Takeaway("AI 时代的软件工程，是把个人产能升级为团队可持续正收益。", 1320, 908)}
  `;
  return svgBaseV5(slide, index, body);
}
...
- \`agents_are_the_new_Agile-v0.2.pptx\`：V0.2 演讲用 PPTX，14 页。
...
- \`agents_are_the_new_Agile-v0.3.pptx\`：V0.3 演讲用 PPTX，14 页。
...
- \`agents_are_the_new_Agile-v0.4.pptx\`：V0.4 演讲用 PPTX，14 页。
- \`agents_are_the_new_Agile-speaker-notes-v0.4.md\`：V0.4 分页讲稿，沿用 15 分钟节奏。
- \`agents_are_the_new_Agile-slides-png-v0.4/\`：V0.4 逐页 SVG 与 PNG。
- \`agents_are_the_new_Agile-preview-contact-sheet-v0.4.png\`：V0.4 缩略总览。
...
- \`agents_are_the_new_Agile-v0.7.pptx\`：V0.7 演讲用 PPTX，${slides.length} 页。
- \`agents_are_the_new_Agile-speaker-notes-v0.7.md\`：V0.7 分页讲稿，沿用 15 分钟节奏。
- \`agents_are_the_new_Agile-slides-png-v0.7/\`：V0.7 逐页 SVG 与 PNG。
- \`agents_are_the_new_Agile-preview-contact-sheet-v0.7.png\`：V0.7 缩略总览。
- \`build-deck-v0.7.cjs\`：V0.7 可复现生成脚本。

重新生成 V0.7：

\`\`\`powershell
node .\\docs\\report\\ai-redefines-software-development-presentation\\build-deck-v0.7.cjs
\`\`\`

V0.7 主线：现实问题 -> 外部证据 -> BitFun 工程控制面 -> 可治理工作流 -> 能力保护式重构 -> 工程协议与职责变化。重点从泛化观点继续下探到 BitFun 当前机制和未来演进路径。
`;
