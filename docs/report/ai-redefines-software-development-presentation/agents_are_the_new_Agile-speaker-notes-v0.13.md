...
各位好，今天分享的主题是《AI 如何重新定义软件开发》。报告会以 BitFun 的一段真实开发经验为入口，但重点不是复盘项目细节，也不是比较哪个工具更强。真正想讨论的是：当 AI 已经能显著放大代码产出以后，软件工程需要怎样升级，才能把这份速度变成稳定的交付能力。今天的主线可以概括成一句话：从代码产能，到可治理 Agentic SDLC。也就是说，AI 不只是帮人写代码，而是逐步进入需求理解、设计、实现、测试、评审、发布和复盘这些环节；相应地，团队要建设上下文、隔离执行、证据包、阶段门禁和反馈学习环，让概率性的执行过程收敛成可判断的工程结果。
...
先看一个入口案例。在 GCWing/BitFun 仓库里，以 2026 年 4 月 22 日到 5 月 22 日为窗口，作者 limityan 在 main 分支上有 241 个提交，新增 185,533 行，删除 46,479 行。可以把它概括成单月 10W+ 行代码，但数字本身不是重点，它更像一个工程压力测试。AI 确实让想法更快变成原型，让个人可以承担过去需要多人探索的工作；但代码更快进入仓库以后，真正的问题会转移到验证覆盖、评审等待、返工闭环和长期维护。换句话说，高吞吐不是净收益的证明，只说明生产环节已经被放大。团队真正要问的是：这些变更凭什么可以合并，证据是否足够，风险能否回滚，后续维护是否还能看懂。AI 放大的不是单个键盘速度，而是从想法到反馈的回路；如果后面的工程承接没有同步升级，速度也可能变成新的压力。
...
从 BitFun 的高吞吐现象往下看，讨论不能只停在“AI 写代码更快”。更值得关注的是，这些行业新概念共同指向了 IDE 边界的外扩。Agentic OS 说明 Agent 正在被当成独立运行主体，需要账号、权限、隔离环境和审计要求。ADE，也就是 Agentic Development Environment，说明开发环境正在从编辑器扩展成多 Agent 的控制平面。龙虾式 Workflow 说明自由对话之外，还需要类型化流程、审批门禁和可恢复执行，让 Agent 不必每一步都自由发挥。Yes Engineer 则是一个更有趣的工程文化信号：它不是让工程师无原则答应需求，而是要求工程师设计一条能安全说 Yes/Go 的路径。放在一起看，未来 IDE 的边界正在从写代码入口，扩展到 Agent 的身份、运行、协作、审批和证据平台；人的角色也从“我来写”更多转向“我来定义边界、证据和放行条件”。
...
- 建议时长：约 1.5 分钟
- 讲述目标：从竞品和研究信号推导 ADE 的产品演进方向：任务委托、治理护栏和证据复盘三类能力最值得优先产品化。
- 可选提问：如果 IDE 边界已经外扩，BitFun/ADE 后续应该补的是哪些系统能力？

讲稿：

如果把这些行业信号拆成产品能力，可以看到三个方向正在变得清晰。第一类是入口与委托：GitHub Agent HQ、Copilot coding agent 和 Codex Cloud 都在把任务入口放到 issue、聊天、CLI、移动端和 IDE 里，执行时进入独立云端环境，结果回到分支和 PR。第二类是治理与护栏：GitHub 继续把 model picker、自检、安全扫描、自定义 agent 和 CLI handoff 加到 coding agent；Claude Code 则用 subagent、权限、hooks、worktree 隔离，把自动执行拆成可配置、可拦截、可恢复的过程。第三类是证据与复盘：Agent session logs、规则上下文、trace、evals 和失败样本库，正在变成团队判断 AI 变更能否合并的依据。AIDev 这类研究已经把几十万级 Agentic PR 作为研究对象，说明未来不只是“工具好不好用”，而是要研究 agent 产出的 PR、评审、失败和协作数据。对 BitFun/ADE 来说，产品方向不应该停留在更大的聊天入口，而是一套可委托、可治理、可复盘的研发系统：任务从哪里来、以什么权限运行、留下什么证据、失败如何回收，都应该成为产品能力。

转场：

有了这些产品化参照，再回到 BitFun/ADE 的研发全链路。

### 第 6 页：ADE 范围

- 建议时长：约 1.2 分钟
- 讲述目标：说明 BitFun/ADE 不只是更强 IDE，而是可以覆盖需求、任务、实现、验证、评审、交付和运维反馈的研发全链路。
- 可选提问：如果 ADE 不只是更强 IDE，它应该覆盖研发链路中的哪些节点？

讲稿：

回到 BitFun，可以把 ADE 理解成一条覆盖研发全过程的工作台。它不是只在编辑器里补代码，而是从需求与场景开始，进入任务契约，再到实现与验证，随后是评审、合并、发布，最后回到运维反馈和下一轮演进。它的价值也不是替代产品、开发、测试或运维中的某个角色，而是在每个节点把 Agent 能力接进工程流。个人原型场景更关注快速验证和可回滚；开源仓库更关注 PR 质量和维护者信任；企业工程更关注权限、合规、跨团队协作和线上稳定。沿着这条链路看，AI 研发系统真正要解决的问题会变得更具体：需求如何变成可执行任务，实现如何留下确定性证据，评审如何治理海量变更，运维反馈如何沉淀成下一轮上下文和规则。
...
最后留下三个更值得继续讨论的问题。第一，IDE 的终点会是什么？如果 Agent 需要身份、权限、隔离环境、上下文和证据，那么 IDE 可能不再只是编辑器，而会成为 Agent 的运行面和工程控制面。第二，信任如何定价？当代码产出可以被快速放大，团队真正稀缺的会变成可合并的证据、可追责的责任边界和可回滚的系统设计。第三，开发者的坐标在哪里？当实现动作越来越多地交给 Agent，人更需要站在问题定义、架构约束、风险解释和最终责任上。今天的核心判断是：AI 的未来不是让软件工程消失，而是迫使软件工程升级。谢谢大家。
