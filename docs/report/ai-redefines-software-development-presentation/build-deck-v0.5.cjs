...
const slidesDir = path.join(outDir, "agents_are_the_new_Agile-slides-png-v0.5");
const pptxPath = path.join(outDir, process.env.PPTX_FILE || "agents_are_the_new_Agile-v0.5.pptx");
const notesPath = path.join(outDir, "agents_are_the_new_Agile-speaker-notes-v0.5.md");
const readmePath = path.join(outDir, "README.md");
const contactSheetPath = path.join(outDir, "agents_are_the_new_Agile-preview-contact-sheet-v0.5.png");
...
function v02Takeaway(textValue, width = 1320, y = 954) {
  ...
  return `
    ...
    <text x="${x + width / 2 + 26}" y="${y + 32}" class="body5" text-anchor="middle" dominant-baseline="middle" style="font-size:27px">${esc(textValue)}</text>
  `;
}
...
function slideShockV02(slide, index) {
  const body = `
    ...
    <rect x="760" y="622" width="862" height="154" rx="18" fill="${L.paper}" stroke="${L.line2}" stroke-width="2" filter="url(#paperShadow)"/>
    <text x="806" y="664" class="small5" style="font-size:22px;fill:${L.orange};font-weight:900">快在哪里：不是“写得多”，而是反馈路径变短</text>
    ${miniFlowV05(824, 704, "想法 / Issue", "隔离执行", "原型 / PR", "验证与评审")}
...
function miniFlowV05(x, y, a, b, c, d) {
  ...
  const nodes = items.map(([nx, label, color]) => `
    <rect x="${nx}" y="${y}" width="${nodeW}" height="${nodeH}" rx="16" fill="${L.bg}" stroke="${color}" stroke-width="2"/>
    <text x="${nx + nodeW / 2}" y="${y + nodeH / 2}" class="small5" text-anchor="middle" dominant-baseline="central" alignment-baseline="middle" style="font-size:19px;fill:${L.ink};font-weight:900">${esc(label)}</text>
  `).join("");
  return `
    ${nodes}
    <path d="M${x + nodeW + 10} ${y + nodeH / 2} H${x + 174}" stroke="${L.blue}" stroke-width="4" marker-end="url(#arrowBlue)"/>
    <path d="M${x + 184 + nodeW + 10} ${y + nodeH / 2} H${x + 386}" stroke="${L.blue}" stroke-width="4" marker-end="url(#arrowBlue)"/>
    <path d="M${x + 396 + nodeW + 10} ${y + nodeH / 2} H${x + 598}" stroke="${L.orange}" stroke-width="4" marker-end="url(#arrowGray)"/>
  `;
}
...
function slideCoverV02(slide, index) {
  const body = `
    ...
    <text x="132" y="396" class="small5" style="font-size:23px;fill:${L.blue};font-weight:900">代码生产放大以后，团队要管理新的交付对象和证据对象</text>
    ${collabRowV05(132, 486, "01", "任务委派", "目标、边界和验收标准先写清楚，Agent 在隔离工作区探索。", L.blue, "terminal")}
    ${collabRowV05(132, 616, "02", "角色分工", "实现、测试、评审、文档可以分离，但权限和上下文要受控。", L.orange, "team")}
    ${collabRowV05(132, 746, "03", "证据放行", "人不审每步操作，而是审差异、测试、风险和责任。", L.blue, "check")}
    ...
    ${v02Takeaway("协作对象从“代码”扩展为任务、Agent、工具执行和可审查证据包。", 1420)}
  `;
  return svgBaseV5(slide, index, body);
}
...
function slideReliabilityV02(slide, index) {
  const body = `
    ...
    <rect x="1286" y="328" width="448" height="52" rx="26" fill="#FFF6EE" stroke="${L.orange}" stroke-width="2"/>
    <text x="1510" y="349" class="small5" text-anchor="middle" dominant-baseline="middle" style="font-size:24px;fill:${L.orange};font-weight:900">0.99^10 ≈ 0.90，长链路需要纠偏</text>
...
function reliabilityCardV05(x, y, n, label, titleValue, chips, color, icon) {
  const chipText = chips.map((chip, i) => {
    const chipW = 170;
    const chipH = 44;
    const cx = x + 44 + (i % 2) * 202;
    const cy = y + 232 + Math.floor(i / 2) * 58;
    return `
      <rect x="${cx}" y="${cy}" width="${chipW}" height="${chipH}" rx="${chipH / 2}" fill="${L.bg}" stroke="${L.line2}" stroke-width="2"/>
      <text x="${cx + chipW / 2}" y="${cy + chipH / 2 - 5}" class="small5" text-anchor="middle" dominant-baseline="middle" style="font-size:21px;fill:${L.ink};font-weight:900">${esc(chip)}</text>
    `;
  }).join("");
  return `
...
function slideExternalSignalsV02(slide, index) {
  const body = `
    ...
    <path d="M300 720 C560 790 820 760 960 714 S1360 672 1620 744" stroke="${L.blue}" stroke-width="5" fill="none" marker-end="url(#arrowBlue)" opacity="0.32"/>
    ${v02Takeaway("速度指标要看净时间、返工率、评审压力和线上风险，而不是只看代码量。", 1420)}
  `;
  return svgBaseV5(slide, index, body);
}
...
function slideLifecycleV02(slide, index) {
  const body = `
    ...
    <text x="960" y="841" class="body5" text-anchor="middle" dominant-baseline="middle" style="font-size:27px"><tspan style="fill:${L.blue};font-weight:900">可执行方向：</tspan><tspan>PR 风险评分 + 证据包 + CI 失败回注 + Trace 归因</tspan></text>
...
function speedCostCardV05(x, y, n, label, titleValue, chips, color, icon) {
  const chipText = chips.map((chip, i) => {
    const chipW = 176;
    const chipH = 44;
    const cx = x + 48 + (i % 2) * 216;
    const cy = y + 232 + Math.floor(i / 2) * 58;
    return `
      <rect x="${cx}" y="${cy}" width="${chipW}" height="${chipH}" rx="${chipH / 2}" fill="${L.bg}" stroke="${L.line2}" stroke-width="2"/>
      <text x="${cx + chipW / 2}" y="${cy + chipH / 2}" class="small5" text-anchor="middle" dominant-baseline="central" alignment-baseline="middle" style="font-size:21px;fill:${L.ink};font-weight:900">${esc(chip)}</text>
    `;
  }).join("");
  return `
    ...
    <text x="${x + 244}" y="${y + 122}" class="small5" text-anchor="middle" dominant-baseline="middle" style="font-size:22px;fill:${color};font-weight:900">${esc(n)} / ${esc(label)}</text>
    <text x="${x + 244}" y="${y + 176}" class="num5" text-anchor="middle" dominant-baseline="middle" style="font-size:34px">${esc(titleValue)}</text>
    ${chipText}
  `;
}
...
function slideQualityV02(slide, index) {
  const body = `
    ...
    <path d="M360 780 C590 832 778 810 960 760 S1340 704 1580 788" stroke="${L.blue}" stroke-width="5" fill="none" marker-end="url(#arrowBlue)" opacity="0.30"/>
...
function slidePredictionV02(slide, index) {
  const body = `
    ${v5Header("04 / 能力转向", "刚入职场：四个能力方向", "AI 会降低写代码门槛，但不会自动补齐判断、验证、理解和协作能力。", 62)}
    ...
    ${v02Takeaway("把“能写代码”升级为“能理解问题并可靠交付”。", 1180)}
  `;
  return svgBaseV5(slide, index, body);
}
...
function writeNotes() {
  ...
  linesOut.push("- Microsoft Research AgentRx（https://www.microsoft.com/en-us/research/blog/systematic-debugging-for-ai-agents-introducing-the-agentrx-framework/）：把 agent 执行轨迹规范化为可验证 trace，用约束检查和失败分类定位关键失败步骤，适合支撑“失败回到系统，而不是只回到人脑”。");
  ...
  linesOut.push("");
  linesOut.push("按正常中文演讲语速，标题页和目录页快速进入主题，核心内容页保持 1.1-1.5 分钟，最后一页用于收束和引出互动。讲稿正文按口播方式组织，可以直接作为演讲者手卡使用。");
  linesOut.push("");
...
- \`agents_are_the_new_Agile-v0.5.pptx\`：V0.5 演讲用 PPTX，${slides.length} 页。
- \`agents_are_the_new_Agile-speaker-notes-v0.5.md\`：V0.5 分页讲稿，沿用 15 分钟节奏。
- \`agents_are_the_new_Agile-slides-png-v0.5/\`：V0.5 逐页 SVG 与 PNG。
- \`agents_are_the_new_Agile-preview-contact-sheet-v0.5.png\`：V0.5 缩略总览。
