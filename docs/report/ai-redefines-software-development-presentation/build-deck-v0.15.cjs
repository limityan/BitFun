...
const slidesDir = path.join(outDir, "agents_are_the_new_Agile-slides-png-v0.15");
const pptxPath = path.join(outDir, process.env.PPTX_FILE || "agents_are_the_new_Agile-v0.15.pptx");
const notesPath = path.join(outDir, "agents_are_the_new_Agile-speaker-notes-v0.15.md");
const readmePath = path.join(outDir, "README.md");
const contactSheetPath = path.join(outDir, "agents_are_the_new_Agile-preview-contact-sheet-v0.15.png");
...
const finalContentV15 = [
  {
    ...
    transition: "先看今天的四个主题，后面会用一个贯穿案例把它们串起来。",
  },
  {
    title: "目录",
    time: "约 0.7 分钟",
    ...
    transition: "先从 BitFun 的实际开发经验进入，看为什么代码量不是终点。",
  ...
  },
  {
    ...
    transition: "这个判断不能只靠单个项目经验，接下来看看行业里正在出现的工作方式变化。",
  ...
  },
  {
    ...
    transition: "接下来假设要做一个 PR 自动摘要能力，看它如何走完整条研发链路。",
    render: slideIndustryPracticesV11,
  },
  {
    title: "案例链路",
    subtitle: "以“PR 自动摘要”为例，看一个功能如何从需求走到反馈。",
    time: "约 1.2 分钟",
    focus: "用一个贯穿功能降低抽象感：PR 自动摘要从需求、任务、实现、评审、发布反馈走完整链路。",
    question: "如果让 AI 做一个 PR 自动摘要功能，怎样才算真正交付完成？",
    script:
    ...
    transition: "先看第一步：一个想法怎样变成 AI 可以执行、团队可以验收的任务。",
    render: slideAdeLifecycleMapV12,
  },
  {
    title: "需求到任务",
    subtitle: "PR 自动摘要不是一句“帮我做一下”，而是目标、非目标、验收与风险边界。",
    time: "约 1.3 分钟",
    focus: "用 PR 自动摘要说明任务契约：把模糊需求转成可执行、可验收、可追责的输入。",
    ...
    transition: "任务契约明确以后，再让 Agent 进入实现和验证循环。",
    render: slideRequirementContractV12,
  },
  {
    title: "实现到验证",
    subtitle: "实现可以探索，验证必须落在可复现证据上。",
    time: "约 1.4 分钟",
    focus: "用 PR 自动摘要说明概率过程与确定性验证：每轮修改都要留下可复现证据。",
  ...
  {
    title: "评审到交付",
    subtitle: "评审不是重看 AI 的每一步，而是检查可读、可追责、可复验的交付物。",
    time: "约 1.4 分钟",
    focus: "用 PR 自动摘要说明 Artifact 包：让维护者或团队负责人敢判断、敢合并、敢回滚。",
    ...
    transition: "代码合并或发布以后，工程链路还没有结束，反馈必须回到下一轮研发系统。",
    render: slideReviewDeliveryV12,
  },
  {
    title: "发布到运维",
    subtitle: "交付不是终点：摘要质量、运行异常和用户反馈要回到下一轮规则与测试。",
    time: "约 1.3 分钟",
    focus: "用 PR 自动摘要说明反馈记忆：线上问题如何回流到规则、测试、架构边界和样本库。",
  ...
  {
    title: "开发者角色",
    subtitle: "AI 降低实现门槛，但提高了问题定义、系统理解、证据判断和责任承接的要求。",
    time: "约 1.7 分钟",
...
slides.forEach((slide, i) => Object.assign(slide, finalContentV15[i]));
...
function slideResponsibilityV10(slide, index) {
  const body = `
    ...
    ${rolePanelV10(108, 390, "团队", "建设可治理交付系统", ["沉淀任务、验证、证据和反馈", "把构建、轨迹、评审做成证据包", "用净时间、返工率、线上风险衡量收益"], L.blue, "team")}
    ${rolePanelV10(690, 390, "个人", "把判断位置前移", ["定义目标、非目标和验收标准", "核验事实与可信上下文", "解释风险并承接责任"], L.orange, "shield")}
    ${rolePanelV10(1272, 390, "新人", "四类能力训练", ["定义问题与用户场景", "读懂系统与运行证据", "设计验证并承担责任"], L.blue, "file")}
    ${v02Takeaway("AI 会降低实现门槛，但会提高问题定义、系统理解、证据判断和责任承接的要求。")}
  `;
  return svgBaseV5(slide, index, body);
}
...
function slideAdeLifecycleMapV12(slide, index) {
  const body = `
    ...
    ${lifecycleStageV12(110, 392, "01", "需求场景", "Reviewer 提效", "看得懂、敢判断", L.blue, "chat")}
    ${lifecycleStageV12(468, 392, "02", "任务契约", "输入 / 非目标 / 验收", "失败与权限边界", L.blue, "file")}
    ${lifecycleStageV12(826, 392, "03", "实现验证", "摘要生成 / 测试 / 日志", "隔离执行与证据", L.orange, "terminal")}
    ${lifecycleStageV12(1184, 392, "04", "评审交付", "PR / Artifact 包", "可信摘要与回滚", L.orange, "check")}
    ${lifecycleStageV12(1542, 392, "05", "反馈演进", "误判 / 性能 / 权限", "回流规则和测试", L.blue, "loop")}
    <rect x="154" y="704" width="1612" height="124" rx="20" fill="${L.paper}" stroke="${L.line2}" stroke-width="2" filter="url(#paperShadow)"/>
    ${projectGrainV12(272, 731, "个人原型", "先跑通效果 / 保持可回滚", L.blue)}
    ${projectGrainV12(772, 731, "开源仓库", "让维护者信任 / 证据随 PR 交付", L.orange)}
    ${projectGrainV12(1272, 731, "企业工程", "权限合规 / 责任清楚 / 可复盘", L.blue)}
    ${v02Takeaway("同一个功能从需求到反馈走完整链路，才能从 AI 产出变成可信交付。")}
  `;
  return svgBaseV5(slide, index, body);
}
...
function slideRequirementContractV12(slide, index) {
  const body = `
    ...
    ${nodePanelV12(116, 398, 430, 360, "输入", "PR 摘要需求", ["Reviewer 想快速理解", "输入来自 PR / Issue / 日志", "不能替人决定合并"], L.blue, "chat")}
    ...
    ${nodePanelV12(1374, 398, 430, 360, "判断", "人机分工", ["人：定义成功与边界", "AI：拆解 / 检索 / 草拟", "系统：记录来源与权限"], L.orange, "team")}
    ${v02Takeaway("任务契约把一句需求变成 Agent 可执行、团队可验收的任务单。")}
  `;
  return svgBaseV5(slide, index, body);
}
...
function slideImplementVerifyV12(slide, index) {
  const body = `
    ...
    ${verifyCardV12(566, 430, "02", "摘要实现", "解析 / 生成 / UI", ["按任务契约实施", "记录输入与改动意图"], L.orange, "terminal")}
    ${verifyCardV12(1014, 430, "03", "验证矩阵", "权限 / 摘要 / 回归", ["覆盖漏摘与误导场景", "失败自动进入修复轮"], L.blue, "check")}
    ${verifyCardV12(1462, 430, "04", "证据包", "样例 / log / risk / impact", ["让评审不用倒推上下文", "保留可复现材料"], L.orange, "file")}
    ...
    ${v02Takeaway("每轮实现都要留下验证证据，才能把概率性探索切成可纠偏的小段。")}
  `;
  return svgBaseV5(slide, index, body);
}
...
function slideReviewDeliveryV12(slide, index) {
  const body = `
    ...
    <text x="960" y="522" class="num5" text-anchor="middle" style="font-size:40px;fill:${L.ink}">Artifact 包 = 让人敢合并的证据</text>
    <path d="M596 562 H1324" stroke="${L.orange}" stroke-width="4" stroke-linecap="round" opacity="0.64"/>
    ${artifactChipV12(626, 602, "摘要样例", L.blue)}
    ...
    ${v02Takeaway("Artifact 包应绑定 PR/发布记录，让人围绕证据判断，而不是重放 AI 每一步。")}
  `;
  return svgBaseV5(slide, index, body);
}
...
function slideOperateLearnV12(slide, index) {
  const body = `
    ...
    ${operateStepV12(138, 430, "01", "灰度发布", "开关 / 回滚 / 限流", L.blue, "shield")}
    ${operateStepV12(554, 430, "02", "质量观测", "采用率 / 误判 / Trace", L.orange, "terminal")}
    ${operateStepV12(970, 430, "03", "复盘学习", "失败样本 / 规则 / 测试", L.blue, "loop")}
    ${operateStepV12(1386, 430, "04", "边界演进", "权限 / 性能 / 体验", L.orange, "cube")}
    ...
    ${learnAssetV12(704, 786, "摘要规则", L.blue)}
    ${learnAssetV12(916, 786, "回归样本", L.orange)}
    ${learnAssetV12(1128, 786, "风险模板", L.blue)}
    ${learnAssetV12(1340, 786, "权限边界", L.orange)}
    ${v02Takeaway("反馈记忆把真实使用问题回流到下一轮规则、测试、权限和体验设计。")}
  `;
  return svgBaseV5(slide, index, body);
}
...
function writeNotes() {
  ...
  linesOut.push("");
  linesOut.push("- AGENTS.md：仓库级规则、平台边界、远程兼容、验证矩阵，支撑第 7 页“任务契约”和第 8 页“验证矩阵”。");
  linesOut.push("- tool_pipeline.rs：可用工具白名单、运行时工具限制、折叠工具规格读取，支撑第 8 页“隔离执行”和“工具轨迹”。");
  linesOut.push("- workspace_paths.rs、miniapp/host_dispatch.rs、miniapp/manager.rs：运行产物根目录防逃逸、小应用能力白名单、高风险权限差异，支撑第 8 页“权限边界”。");
  linesOut.push("- docs/architecture/core-decomposition.md 与 scripts/check-core-boundaries.mjs：产品能力保护、端口和提供者迁移、边界检查与等价验证，支撑第 10 页“边界演进”。");
  linesOut.push("- product-domains MiniApp/function-agent contract tests：用契约测试和快照保护 owner 迁移后的行为等价，支撑第 10 页“反馈回归”和“回归样本”。");
  linesOut.push("");
...
本目录保留原版演讲材料、V0.2、V0.3、V0.4、V0.5、V0.6、V0.8、V0.9、V0.10、V0.11、V0.12、V0.13、V0.14 版本，并新增 V0.15 版本。V0.15 继续使用整页图片式生成链路，不做元素化编辑版改造；本版面向更强的大三学生或准新人，保留专业框架，同时用“PR 自动摘要”作为贯穿案例串联需求、实现、验证、评审、发布和反馈，降低抽象概念密度并强化推理链。
