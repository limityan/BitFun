...
const slidesDir = path.join(outDir, "agents_are_the_new_Agile-slides-png-v0.16");
const pptxPath = path.join(outDir, process.env.PPTX_FILE || "agents_are_the_new_Agile-v0.16.pptx");
const notesPath = path.join(outDir, "agents_are_the_new_Agile-speaker-notes-v0.16.md");
const readmePath = path.join(outDir, "README.md");
const contactSheetPath = path.join(outDir, "agents_are_the_new_Agile-preview-contact-sheet-v0.16.png");
...
V0.16 主线：回到 V0.14 的整体结构，校准目录与页面章节为“现实观察、行业信号、工程治理、开发者角色”；同时将“信任如何定价”调整为“信任如何量化”，并把任务契约、Artifact 包等术语补充为“可执行任务说明”“Artifact 证据包”等更容易理解的表达。
`;
