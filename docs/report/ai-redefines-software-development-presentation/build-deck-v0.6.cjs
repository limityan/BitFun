...
const slidesDir = path.join(outDir, "agents_are_the_new_Agile-slides-png-v0.6");
const pptxPath = path.join(outDir, process.env.PPTX_FILE || "agents_are_the_new_Agile-v0.6.pptx");
const notesPath = path.join(outDir, "agents_are_the_new_Agile-speaker-notes-v0.6.md");
const readmePath = path.join(outDir, "README.md");
const contactSheetPath = path.join(outDir, "agents_are_the_new_Agile-preview-contact-sheet-v0.6.png");
...
const slides = [
  {
    ...
    question: "当代码生成不再是瓶颈，软件工程真正稀缺的是什么？",
    script:
  ...
  },
  {
    ...
    subtitle: "最新信号不只讨论生产率，也开始转向架构选择、生产测量、平台治理和质量成本。",
    time: "约 1.3 分钟",
  ...
  },
  {
    ...
    title: "先进观点：能力不只在模型，也在运行基座",
    subtitle: "Context、Tools、Harness、Guardrails 共同决定 Agent 能否可靠交付。",
    time: "约 1.3 分钟",
    focus: "引入 Agent Harness、AgentOps、Context Engineering 等观点，把能力从模型迁移到工程基座。",
    ...
    transition: "有了运行基座，再看 Agent 执行，就能理解为什么过程和结果要分开治理。",
  ...
  },
  {
    ...
    subtitle: "质量要求不能停留在口头承诺里，必须变成可复核工件。",
    time: "约 1.3 分钟",
    focus: "区分开源与大厂场景，但落到同一个核心：质量要求必须变成可复核工件。",
    question: "AI 让提交变多以后，维护者和平台团队到底该看什么？",
    script:
    ...
    transition: "这些链路再往前走，就会变成可执行的工程协议。",
    render: slideQualityV02,
  },
  {
    section: "05 / QUALITY GOVERNANCE",
...
function slideTitleV02(slide, index) {
  const body = `
    ...
    <g opacity="0.22">
      <path d="M1110 406 C1248 260 1372 516 1518 364 S1690 300 1768 488" stroke="${L.blue}" stroke-width="5" fill="none" stroke-linecap="round"/>
      <path d="M1188 308 C1308 430 1450 286 1608 404" stroke="${L.orange}" stroke-width="3.5" fill="none" stroke-linecap="round"/>
      <path d="M1278 572 H1626" stroke="${L.line2}" stroke-width="3" fill="none" stroke-linecap="round"/>
      <path d="M1170 496 C1290 444 1402 512 1526 462" stroke="${L.blue}" stroke-width="2.5" fill="none" stroke-linecap="round" opacity="0.55"/>
    </g>
...
function slideExplorationV02(slide, index) {
  const body = `
    ...
    <rect x="146" y="408" width="704" height="382" rx="18" fill="${L.paper}" stroke="${L.line2}" stroke-width="2" filter="url(#paperShadow)"/>
    <rect x="1070" y="408" width="704" height="382" rx="18" fill="${L.paper}" stroke="${L.line2}" stroke-width="2" filter="url(#paperShadow)"/>
    <text x="202" y="466" class="small5" style="fill:${L.blue};font-weight:900">Context Engineering</text>
    <text x="202" y="528" class="num5" style="font-size:45px">让 Agent 站在正确事实上</text>
    ${v02Lines(["需求 / 架构 / AGENTS.md / 历史决策", "测试、日志、运行约束和 owner 信息", "上下文质量决定生成质量上限"], 202, 604, 27, L.muted, 40, 680)}
    <text x="1126" y="466" class="small5" style="fill:${L.orange};font-weight:900">Harness / AgentOps</text>
    <text x="1126" y="528" class="num5" style="font-size:42px">重点不是工具清单，是控制面</text>
    ${harnessFocusV06(1132, 600, "01", "隔离权限", "限制工具、数据和分支影响面", L.orange)}
    ${harnessFocusV06(1132, 672, "02", "观测证据", "沉淀 trace、日志、测试和风险", L.blue)}
    ${harnessFocusV06(1132, 744, "03", "失败回注", "CI 失败和复盘进入下一轮", L.orange)}
    ...
    <text x="960" y="842" class="num5" text-anchor="middle" style="font-size:34px;fill:${L.blue}">模型能力 + 运行基座 = 可治理交付</text>
    ${v02Takeaway("先进实践的共同方向：能力不只在模型，也在上下文、工具、验证和治理基座。", 1460)}
  `;
  return svgBaseV5(slide, index, body);
}
...
function slideExternalSignalsV02(slide, index) {
  const body = `
    ...
    ${sourceSignalCardV07(1050, 626, 650, 204, "MSR 2026", "质量成本", "速度收益会被吞回", ["告警、重复代码、复杂度上升"], L.orange, "file")}
    <rect x="236" y="864" width="1448" height="62" rx="14" fill="${L.bg}" stroke="${L.line2}" stroke-width="2"/>
    <text x="960" y="895" class="small5" text-anchor="middle" dominant-baseline="central" alignment-baseline="middle" style="font-size:25px;fill:${L.ink};font-weight:900">核心判断：AI 研发收益 = 架构适配 + 生产可控 + 平台治理 + 质量成本约束</text>
...
function slideQualityV02(slide, index) {
  const body = `
    ${v5Header("05 / 工程质量", slide.title, slide.subtitle, 64)}
    ...
    <text x="960" y="424" class="small5" text-anchor="middle" style="font-size:22px;fill:${L.muted};font-weight:900">共同检查面</text>
    ...
    ${v02Takeaway("质量治理不是多一道审批，而是把责任、证据和工程约束变成可复核工件。", 1480)}
  `;
  return svgBaseV5(slide, index, body);
}
...
function slideThanksV02(slide, index) {
  const body = `
    ...
    ${discussionPointV06(248, 794, "MSR 2026", "速度收益背后的质量债和维护成本由谁度量？", L.blue)}
    ...
    ${v02Takeaway("AI 时代的软件工程，是从经验流程走向可验证、可治理、可复盘系统。", 1320, 908)}
  `;
  return svgBaseV5(slide, index, body);
}
...
本目录保留原版演讲材料、V0.2、V0.3、V0.4、V0.5 版本，并新增 V0.6 版本。V0.6 继续使用整页图片式生成链路，但重新排列主线：从代码产能被 AI 放大后的工程压力进入，结合外部研究背书，收敛到运行基座、证据包、风险分流、阶段门禁和责任边界这套可落地框架。
...
- \`agents_are_the_new_Agile-v0.6.pptx\`：V0.6 演讲用 PPTX，${slides.length} 页。
- \`agents_are_the_new_Agile-speaker-notes-v0.6.md\`：V0.6 分页讲稿，沿用 15 分钟节奏。
- \`agents_are_the_new_Agile-slides-png-v0.6/\`：V0.6 逐页 SVG 与 PNG。
- \`agents_are_the_new_Agile-preview-contact-sheet-v0.6.png\`：V0.6 缩略总览。
- \`build-deck-v0.6.cjs\`：V0.6 可复现生成脚本。

重新生成 V0.6：

\`\`\`powershell
node .\\docs\\report\\ai-redefines-software-development-presentation\\build-deck-v0.6.cjs
\`\`\`

V0.6 主线：现实问题 -> 外部证据 -> 工程基座 -> 可治理工作流 -> 质量协议 -> 职责变化。重点从泛化观点转为可执行落地方案，并保留 DORA、METR、Harness、Agent Harness、AgentOps 等研究和技术背书。
`;
