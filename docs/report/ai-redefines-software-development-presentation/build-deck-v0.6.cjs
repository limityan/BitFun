...
const slidesDir = path.join(outDir, "slides-png-v0.6");
const pptxPath = path.join(outDir, process.env.PPTX_FILE || "ai-redefines-software-development-v0.6.pptx");
const notesPath = path.join(outDir, "speaker-notes-v0.6.md");
const readmePath = path.join(outDir, "README.md");
const contactSheetPath = path.join(outDir, "preview-contact-sheet-v0.6.png");
...
- \`ai-redefines-software-development-v0.6.pptx\`：V0.6 演讲用 PPTX，${slides.length} 页。
- \`speaker-notes-v0.6.md\`：V0.6 分页讲稿，沿用 15 分钟节奏。
- \`slides-png-v0.6/\`：V0.6 逐页 SVG 与 PNG。
- \`preview-contact-sheet-v0.6.png\`：V0.6 缩略总览。
