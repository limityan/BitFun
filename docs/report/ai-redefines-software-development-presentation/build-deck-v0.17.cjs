...
const slidesDir = path.join(outDir, "agents_are_the_new_Agile-slides-png-v0.17");
const pptxPath = path.join(outDir, process.env.PPTX_FILE || "agents_are_the_new_Agile-v0.17.pptx");
const notesPath = path.join(outDir, "agents_are_the_new_Agile-speaker-notes-v0.17.md");
const readmePath = path.join(outDir, "README.md");
const contactSheetPath = path.join(outDir, "agents_are_the_new_Agile-preview-contact-sheet-v0.17.png");
...
function backgroundSvgWithoutEditableText(svg) {
...
function editableTextEntries(svg) {
  ...
  while ((match = SVG_TEXT_RE.exec(svg))) {
    ...
    let rawY = numericPx(svgAttr(attrs, "y"), 0);
    ...
    if (isMiddleBaseline) rawY -= fontPx * 0.56;
...
async function renderImages() {
  for (const entry of fs.readdirSync(slidesDir)) {
    if (/^slide-\d+(?:-bg)?\.(svg|png)$/.test(entry)) {
  ...
  for (let i = 0; i < slides.length; i += 1) {
    ...
    const bgSvg = backgroundSvgWithoutEditableText(svg);
    ...
    const bgSvgPath = path.join(slidesDir, `slide-${String(i + 1).padStart(2, "0")}-bg.svg`);
    const bgPngPath = path.join(slidesDir, `slide-${String(i + 1).padStart(2, "0")}-bg.png`);
    fs.writeFileSync(svgPath, svg, "utf8");
    fs.writeFileSync(bgSvgPath, bgSvg, "utf8");
    await sharp(Buffer.from(svg)).png().toFile(pngPath);
    await sharp(Buffer.from(bgSvg)).png().toFile(bgPngPath);
    pngPaths.push(pngPath);
    bgPngPaths.push(bgPngPath);
    svgs.push(svg);
  }
  return { pngPaths, bgPngPaths, svgs };
}
...
async function writePptx(rendered) {
  ...
  rendered.bgPngPaths.forEach((pngPath, i) => {
