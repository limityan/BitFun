...
const slidesDir = path.join(outDir, "agents_are_the_new_Agile-slides-png-v0.4");
const pptxPath = path.join(outDir, process.env.PPTX_FILE || "agents_are_the_new_Agile-v0.4.pptx");
const notesPath = path.join(outDir, "agents_are_the_new_Agile-speaker-notes-v0.4.md");
const readmePath = path.join(outDir, "README.md");
const contactSheetPath = path.join(outDir, "agents_are_the_new_Agile-preview-contact-sheet-v0.4.png");
...
const slides = [
  ...
  },
  {
    ...
    time: "约 1.4 分钟",
    focus: "承接 BitFun 案例，说明代码产出放大后，协作对象从代码扩展到任务委派、角色分工、证据放行。",
...
function slideCoverV02(slide, index) {
  const body = `
    ...
    <path d="M680 468 V776 M1240 468 V776" stroke="${L.line2}" stroke-width="2" stroke-dasharray="9 12"/>
    ${collabLaneV02(206, 506, "01", "任务委派", "目标 / 工作区", "人定义目标和边界", "Agent 独立探索与修改", L.blue, "terminal")}
    ${collabLaneV02(766, 506, "02", "角色分工", "实现 / 测试 / 评审", "Agent 分工产出中间结果", "工具权限和生命周期受控", L.orange, "team")}
    ${collabLaneV02(1326, 506, "03", "证据放行", "PR / CI / Gate", "人看证据包和风险", "承担合并或发布责任", L.blue, "check")}
...
- \`agents_are_the_new_Agile-v0.4.pptx\`：V0.4 演讲用 PPTX，${slides.length} 页。
