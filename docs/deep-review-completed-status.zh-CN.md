...
最新 M3 consolidation 记录了聚焦 web 验证和静态 stale-claim / privacy 检查。完整 Rust 验证延期到合并里程碑验证批次。
...
当前已完成边界明确不包含 backend-owned automatic redispatch scheduler、backend batch/stagger scheduling、project-level cache、hard byte clipping、programmatic shared tool-result reuse、global subagent scheduling 和进一步的大型架构重构。
