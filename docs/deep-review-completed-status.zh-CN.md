...
- 本地 reviewer queue liveness 已收口：同一次 Deep Review 仍有 active reviewer 时继续等待，不把固定短等待窗口当作每个 reviewer 的过期时间。
...
当前已完成边界明确不包含 backend-owned automatic redispatch scheduler、完整 backend batch/stagger scheduling、project-level cache、hard byte clipping、programmatic shared tool-result reuse、global subagent scheduling 和进一步的大型架构重构。本地 queue liveness 已在当前 reviewer admission 路径完成；当前 launch-batch 保护只是重叠安全网，不是完整 dispatcher。
