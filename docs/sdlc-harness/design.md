...
组装边界；SDLC Harness 能力应通过 contracts、execution primitives、services、adapters 和
...
| Lifecycle Harness Control Plane | execution 中的 `bitfun-harness` / harness provider，Product Capability 中的 harness pack | 定义 workflow、gate、policy、post-processor；通过 Product Assembly 注册，不访问 session manager internals 或 concrete IO |
| Artifact and Evidence Plane | contracts / product-domains 的 DTO、port、artifact contract，harness artifact projection | 定义证据、finding、风险接受和图谱关系；只引用事实，不直接执行 Git、CI 或终端操作 |
...
services/adapters 的目标项目集成；随后实现 harness provider、risk/gate/impact/eval 等编排；最后通过
...
Harness contract、Agent Runtime SDK 或稳定契约层。
