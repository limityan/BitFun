...
- 剩余 PR 数量以 `docs/plans/core-decomposition-plan.md` 的“低风险准备 PR 合并锁定
  （2026-05-19）”为准：当前 product-domain owner-helper PR 合入后，低风险准备工作
  合并为 `LR1` 一个 PR；LR1 已闭环为文档/边界/基线校准，不包含任何 runtime owner
  迁移，之后才开始高风险 runtime 迁移队列。后续实现必须先按
  `LR1` / `H1` / `H2` / `H3` / `H4` / `H5` 对齐文档，不能边做边重新拆分，也不能把
  `H5` 的 `bitfun-core default = []` / per-product feature matrix / 构建收益评估混入
  runtime owner 迁移。
- LR1 的当前结论是：`BitFunError` / `BitFunResult`、remote-connect dialog、cron、
  remote-SSH runtime、`ToolUseContext`、runtime manifest assembly / `GetToolSpec` 执行、
  concrete tools、MiniApp IO/process/host/builtin runtime 与 function-agent Git/AI runtime
  都显式保留 core-owned/deferred；后续只能在 H1/H2/H3 中按单一 owner 审核迁移。
