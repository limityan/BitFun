...
- 剩余 PR 数量以 `docs/plans/core-decomposition-plan.md` 的“剩余 PR 计数锁定（2026-05-19）”
  为准：当前 product-domain owner-helper PR 合入后，主线还剩 R1-R4 四个必需 PR；
  若把 `bitfun-core default = []` / per-product feature matrix / 构建收益也计入“全部完成”，
  则另有 R5 一个可选评估 PR。后续实现必须先按 R1-R4 顺序对齐文档，不能边做边重新拆分
  或把 R5 混入 runtime owner 迁移。
