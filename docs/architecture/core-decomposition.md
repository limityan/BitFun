...
- 后续 runtime 迁移以 `docs/plans/core-decomposition-plan.md` 的里程碑表为准，不再按零散
  ...
  - H1：tool runtime owner 迁移。当前只完成迁移前/迁移中边界收敛：`bitfun-agent-tools`
    ...
    contextual manifest resolver、GetToolSpec presentation/schema/detail helper；
    core 仍持有 `ToolUseContext`、runtime manifest assembly、`GetToolSpecTool`
    执行、collapsed unlock state、snapshot decorator 与 concrete tools。
    当前 PR 进一步把 core `Tool` 到 provider-neutral contract 的 adapter 收敛到
    `tool_adapter.rs`，把 product catalog / manifest / GetToolSpec catalog-detail facade
    收敛到 `catalog_provider.rs`；这是 H1 前的 core 内部 owner closure，不改变工具行为。
