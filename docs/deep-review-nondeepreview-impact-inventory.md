...
|---|---|---|---|
| `src/crates/core/src/agentic/tools/implementations/task_tool.rs` | Deep Review adapter extraction, queue/capacity handling, retry admission, retry guidance and packet/cache gates | Normal Task could accidentally enter Deep Review queue/retry/cache behavior or receive Deep Review retry guidance | Rust non-DeepReview Task and Deep Review focused tests were added; final release gate passed `cargo test -p bitfun-core deep_review -- --nocapture` and `cargo check --workspace --exclude bitfun-cli`; post-review regression `deep_review_retry_guidance_only_applies_to_initial_reviewer_timeout` keeps retry guidance Deep Review-gated. |
| `src/crates/core/src/agentic/tools/implementations/code_review_tool.rs` | Deep Review packet metadata, reliability signals, cache write-through, evidence-pack validation signal | Standard Code Review could receive Deep Review-only report metadata | Rust report tests cover standard submission and Deep Review enrichment boundaries; final release gate passed `cargo test -p bitfun-core deep_review -- --nocapture` and `cargo check --workspace --exclude bitfun-cli`. |
| `src/crates/core/src/agentic/deep_review/*` | Deep Review subsystem ownership for policy, queue, diagnostics, manifest, report, task adapter and cache | Future contributors could bypass explicit Deep Review gates | Module-level tests and facade compatibility tests were added; final release gate passed `cargo test -p bitfun-core deep_review -- --nocapture` and `cargo check --workspace --exclude bitfun-cli`. |
...
| `src/web-ui/src/flow_chat/deep-review/action-bar/*` and `src/web-ui/src/flow_chat/components/btw/DeepReviewActionBar.tsx` facade | Deep Review capacity queue notice, review action header, and formatting helpers are split from the shared action-bar component | Standard Code Review action bar could show Deep Review queue controls or lose remediation behavior | `pnpm --dir src/web-ui exec vitest run src/flow_chat/deep-review/action-bar/CapacityQueueNotice.test.tsx src/flow_chat/deep-review/action-bar/ReviewActionHeader.test.tsx src/flow_chat/deep-review/action-bar/actionBarFormatting.test.ts src/flow_chat/components/btw/DeepReviewActionBar.test.tsx`; standard review remediation test remains covered. |
