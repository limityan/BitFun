...
| Backend batch / stagger scheduling | 不在本计划内实现。 | 降低 provider 瞬时并发尖峰；改善 capacity 成功率；更容易做公平调度和渐进反馈。 | 总完成时间可能变长；用户等待感变差；失败恢复顺序复杂；与 active-session warning、manual continuation、queue controls 交叠。 | 启动节奏、最大批次、stagger 默认值、用户等待文案、失败恢复和公平性策略。 |
...
|---|---:|---|---|---|---|
| Backend batch / stagger scheduling | P0 | 先做“轻量 stagger”：默认小批次启动 reviewer，action bar 显示等待/已启动/未启动计数；高级参数暂不暴露。 | 最直接降低 provider burst 和 capacity failure，用户能看到渐进进展。 | 总耗时可能变长；用户可能误解为系统变慢；失败顺序和 retry merge 更复杂。 | 推荐优先设计。它是 capacity 体验的关键补强，但必须先定默认批次、等待文案、取消/跳过语义。 |
...
- Backend batch / stagger scheduling 是最值得优先设计的延期项，收益集中在 capacity 成功率和等待可解释性；风险集中在 wall-clock 变长、失败顺序和用户感知变慢。
