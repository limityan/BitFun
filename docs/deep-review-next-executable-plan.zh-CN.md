...
- 完整 backend batch / stagger scheduler 仍未实现；当前已补 P0-a 批次重叠保护：TaskTool 会在前序 `launch_batch` 仍有活跃 reviewer 时把后续批次放入可见 capacity queue，并给出 `launch_batch_blocked` 原因；后续批次不会在前序 reviewer 仍活跃时因固定短等待窗口跳过。
- Deep Review local queue liveness policy 已完成 P0 修正：短队列等待上限不再代表“每个排队 reviewer 只等 60 秒”。当同一次 Deep Review 内仍有任意 reviewer 正在执行或可执行时，后续 reviewer 持续等待；只有没有任何 Deep Review reviewer 在执行、且仍无法取得可执行通路时，整体队列超时才有产品意义。
...
| 子计划 | 目标 | 主要文件 | 行为变化 | 最小验证 | 退出标准 |
|---|---|---|---|---|---|
| M2-P0 Queue liveness semantics | 已完成：修正 Deep Review local queue 活性语义，取消“每个排队 reviewer 固定短超时”的产品口径 | `deep_review/task_adapter.rs`、`deep_review/concurrency_policy.rs`、`budget.rs`、`deepReviewCapacityGuard.ts`、action-bar queue helpers | 是，排队等待更符合 Deep Review 完整性 | Rust queue liveness tests；action bar queue state tests | active reviewer 存在时排队 reviewer 不因 60s 被跳过；无 active reviewer 且无可执行通路才整体超时；主 session / 多 session subagent 不计入 Deep Review cap。 |
...
- 产品结论：这是 P0，因为当前 60 秒 local queue timeout 会让排队 reviewer 在合理等待场景中被错误跳过，直接损害 Deep Review 完整性。
- 当前状态：已完成后端 local queue liveness 修正；`QueueExpired` 只在没有 active Deep Review reviewer 且仍无法取得可执行通路时成立。用户主动 cancel / optional skip 仍立即生效。
- 已验证：`cargo test -p bitfun-core deep_review_capacity_queue -- --nocapture`，覆盖 active reviewer liveness、launch batch liveness、pause / continue、cancel、skip optional 和 runtime wait diagnostics。
- 前置检查：确认当前 Deep Review active reviewer 计数按 `parent_dialog_turn_id` 隔离；确认 `deepReviewCapacityGuard.ts` 只作为启动前提示，不应成为阻塞 Deep Review 的 hard gate。
- 改动范围：Deep Review local queue admission / timeout 判断、queue event reason、action bar 展示、Review Team settings 文案。
- 必须验证：
  ...
  - `active_reviewer_count == 0` 且无法取得 Deep Review 可执行通路时，队列进入 `no_executable_path_timeout` 或等价稳定原因，并给出可操作错误。
...
以下事项不属于本文四个里程碑的实现范围。每项都必须独立形成产品/架构决策，不能作为 cleanup 或 refactor 的顺手行为合入。
...
|---|---:|---|---|---|---|
| Deep Review queue liveness semantics | 已完成 | 已作为 M2-P0 落地：Deep Review 内部排队不再按每个 reviewer 固定 60 秒短超时；当同一次 Deep Review 仍有 active reviewer 时持续等待；只有没有任何 active reviewer 且没有可执行通路时才整体超时。 | 避免弱模型/低并发下大量 reviewer 被错误跳过，保持 Deep Review 完整性和用户信任；错误原因更真实。 | 如果 active guard 泄漏或 running reviewer 没有 watchdog，可能造成等待过久；需要继续保留可见 queue state、cancel / optional skip 控制和 focused regression。 | 不再作为延期决策；继续作为 provider queue、stagger、retry 的前置回归门禁。 |
...
- Queue liveness semantics 已从待决策项转为已落地基线；后续 provider queue / retry / stagger 只能建立在“active reviewer 存在时 local queue 持续等待”的语义上。
