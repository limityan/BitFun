...
- Deep Review queue liveness policy 仍需产品化修正：当前短队列等待上限不应被理解为“只等某个排队 reviewer 60 秒”。当同一次 Deep Review 内仍有任意 reviewer 正在执行或可执行时，后续 reviewer 应持续等待；只有当没有任何 Deep Review reviewer 在执行、且仍无法取得可执行通路时，整体队列超时才有产品意义。
...
|---|---:|---|---|---|---|
| Deep Review queue liveness semantics | P0 | 立即作为 M2-P0 修正：Deep Review 内部排队不再按每个 reviewer 固定 60 秒短超时；当同一次 Deep Review 仍有 active reviewer 时持续等待；只有没有任何 active reviewer 且没有可执行通路时才整体超时。 | 避免弱模型/低并发下大量 reviewer 被错误跳过，保持 Deep Review 完整性和用户信任；错误原因更真实。 | 如果 active guard 泄漏或 running reviewer 没有 watchdog，可能造成等待过久；需要 UI 明确展示正在阻塞的 reviewer 和可取消操作。 | 推荐最高优先级先做；它是后续 provider queue、stagger、retry 的前置语义基础。 |
...
- 本轮新增的 queue liveness semantics 不是普通延期项，而是 M2 的优先前置修正；它应先于继续扩大 provider queue / retry 行为，否则后续优化会建立在错误的等待语义上。
